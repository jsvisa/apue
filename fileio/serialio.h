 ...
 * @on - 1-硬件开，0-硬件关
 */
extern void setHardwareFlow(int fd, int on);

//extern int setRTS(int fd);

/** 保存串口当前状态
 * @fd 串口文件描述符
 */
extern void saveState(int fd);

/** ????????
 * @fd - 文件描述符
 * @sec - 秒数
 */
extern void DTRToggle(int fd, int sec);

/** 中断终端
 */
extern void breakTerm(int fd);

/** 恢复串口设置
 * @savetty - 保存的串口参数
 */
extern void restoreState(int fd);

/** 设置非阻塞
 */
extern void noHang(int fd);

/** 在关闭的时候hup
 */
extern void hupOnClose(int fd, int on);
 ...
 * @hwf - 硬件,0-无,1-启用
 * @swf - 软件
 * @return 函数值返回0表示成功，否则返回非0值
 */
extern int setParams( int fd, long baudr, char par, int bits, int stopb, int hwf, int swf);
 ...
 * @return 返回读到的字节数，或-1
 */
extern int timeRead(int fd, char *buf, int size, int timeout);

/** 设置CBreak模式
 */
extern int setCBreakMode(int fd, int mode);
